...
class Documenter:
    def write_readme(self, with_source_ref=False, make_toc=False):
        """
        ...
        fnr = self.cfg['fn_target_md_tmpl']
        with open(fnr) as fd:
            readm = fd.read()
        ...
        md = ''.join((pre, m, tut, '\n', m, post))
        print('Now postprocessing', self.cfg['fn_target_md'])
        d = dirname(self.cfg['fn_target_md'])
        ...
        if make_toc:
            md = mdt.make_toc(md)
        with open(self.cfg['fn_target_md'], 'w') as fd:
            fd.write(md)

    def run_and_document_pyfunc(self, funcstr, frame):
        ...
        s = python(s)
        del self.printed.stdout[:]  # py2, no clear
        del self.printed.stderr[:]  # py2, no clear
        try:
            o = sys.stdout
            e = sys.stderr
    ...
    def md_from_source_code(self):
        ...
        while parts:
            ...
            if parts:
                ...
                try:
                    funcname = (
                        func.lstrip().split('def ', 1)[1].split('(', 1)[0]
                    )
                except Exception as ex:
                    continue
                part += '\npyrun: %s\n' % funcname
            r += part
        self.md(r, test_func=src)

    def md(self, paras, into=nothing, test_func=None):
        """writes markdown into our intermediate file"""
        paras = [paras]
        lctx = {}
        ...
        while parts:
            ...
            after += self.run_and_document_pyfunc(func, test_func) + post
    ...
    def bash_run(self, cmd, res_as=None, no_cmd_path=False, no_show_in_cmd=''):
        ...
        self.md(r, into=res_as if res_as else bash)
        return cmds

    def sh_file(self, fn, lang='python', content=None):
        ex = exists(fn)
    ...
    def sh_code(self, func):
        return self.md(python(inspect.getsource(func)))
...
class Tools:
    ...
    def html_table(self, list, headers, summary=None):
    ...
    def convert_to_staticmethods(self, cls):
        ...
        [
            setattr(cls, k, staticmethod(f.__func__ if PY2 else f))
            for k, f in meths
        ]


class P2M(Documenter, Tools):
    def __init__(
        self,
        fn_test_file,
        fn_target_md=None,
        ...
        (Pdb) pp self.cfg (for a run of our own tutorial)
        ...
        self.cfg['md_sep'] = md_sep
        self.cfg['here'] = here = DIR(fn_test_file)
        ...
        if not fn_target_md.startswith('/'):
            fn_target_md = abspath(self.cfg['here'] + '/' + fn_target_md)
        fn = self.cfg['fn_target_md'] = fn_target_md
        ...
        if not fn_target_md_tmpl:
            print(
                ...
                self.cfg['fn_target_md'],
            )
            fn_target_md_tmpl = self.cfg['fn_target_md']
        self.cfg['fn_target_md_tmpl'] = fn_target_md_tmpl
        ...
        name = self.cfg['name'] = rpl(
            fn_test_file.rsplit('/', 1)[-1], 'test_', '.py'
        )
        self.cfg['d_assets'] = here + '/' + name + '/'

        def create_empty_md_file(name):
            here = self.cfg['here']
            # create
            fn_md = self.cfg['fn_md'] = here + '/' + name + '.md'
            if exists(fn_md):
                os.unlink(fn_md)
